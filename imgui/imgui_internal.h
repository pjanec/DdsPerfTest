...
// This is used by SharedData to store persistent data while reducing allocation count.
...
struct ImGuiContext
{
    ...
    // SharedData
    bool                    SettingsLoaded;
    float                   SettingsDirtyTimer;                 // Save .ini SharedData to memory when time reaches zero
...
enum ImGuiTabBarFlagsPrivate_
{
    ...
    ImGuiTabBarFlags_SaveSettings               = 1 << 22,  // FIXME: SharedData are handled by the docking system, this only request the tab bar to mark settings dirty when reordering tabs
};
